...
namespace SettingsModelLocalTests
{
    ...
    void DeserializationTests::TestLayerUserDefaultsBeforeProfiles()
    {
...

        {
            ...
            settings->_ParseJsonString(settings0String, false);
            VERIFY_IS_NULL(settings->_baseLayerProfile);
            settings->_ApplyDefaultsFromUserSettings();
            VERIFY_IS_NOT_NULL(settings->_baseLayerProfile);
            settings->LayerJson(settings->_userSettings);
    ...
    void DeserializationTests::TestCloneInheritanceTree()
    {
        ...
        VERIFY_ARE_EQUAL(settings->_baseLayerProfile->Name(), copyImpl->_baseLayerProfile->Name());

        // Modifying profile.defaults should...
        VERIFY_ARE_EQUAL(settings->_baseLayerProfile->HasName(), copyImpl->_baseLayerProfile->HasName());
        copyImpl->_baseLayerProfile->Name(L"changed value");
        ...
        VERIFY_ARE_EQUAL(settings->_baseLayerProfile->HasName(), copyImpl->_baseLayerProfile->HasName());
        VERIFY_ARE_NOT_EQUAL(settings->_baseLayerProfile->Name(), copyImpl->_baseLayerProfile->Name());
        ...
        auto verifyEmptyPD = [this](const std::string json) {
            ...
            VERIFY_IS_NOT_NULL(settings->_baseLayerProfile);
            VERIFY_IS_NOT_NULL(copyImpl->_baseLayerProfile);
