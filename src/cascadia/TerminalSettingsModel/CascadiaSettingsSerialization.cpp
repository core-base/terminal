...
void SettingsLoader::LayerGeneratedProfiles()
{
...
void SettingsLoader::LayerFragments()
{
...
try
{
    ...
    loader.LayerGeneratedProfiles();
    // Fragments might reference profiles generated by a generator.
    // --> _layerFragments is called after _layerGeneratedProfiles.
    loader.LayerFragments();
...
{
    SettingsLoader loader{ DefaultUniversalJson, "{}" };
    return *winrt::make_self<CascadiaSettings>(std::move(loader));
}
...
{
    SettingsLoader loader{ DefaultJson, "{}" };
    return *winrt::make_self<CascadiaSettings>(std::move(loader));
}
